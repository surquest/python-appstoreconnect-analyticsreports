...
class Client:
    ...
    def _get_resource(self, resource_path: str, params: Optional[Dict[str, Any]] = None) -> Optional[Dict[str, Any]]:
        """Builds URL and fetches resource."""
        return self._get_request(f"{self.BASE_URL}/{resource_path}", params)

    def _paginate(self, resource_path: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    def read_report_requests(self, app_id: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        return self._paginate(f"apps/{app_id}/analyticsReportRequests", params)

    def read_report_for_specific_request(self, request_id: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        return self._paginate(f"analyticsReportRequests/{request_id}/reports", params)

    def read_list_of_instances_of_report(self, report_id: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        return self._paginate(f"analyticsReports/{report_id}/instances", params)

    def read_segments_for_report(self, instance_id: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        return self._paginate(f"analyticsReportInstances/{instance_id}/segments", params)
    ...
    def _download_gzipped_csv(self, url: str) -> str:
        ...
        response = self.session.get(url, headers={"Accept-Encoding": "gzip"}, stream=True)
        response.raise_for_status()
        decompressed_content = gzip.decompress(response.content)
        return decompressed_content.decode('utf-8')

    def _parse_csv_to_dicts(self, csv_content: str, normalize: bool) -> List[Dict[str, str]]:
        """Parses CSV content into list of dictionaries."""
        csv_file = io.StringIO(csv_content)
        reader = csv.DictReader(csv_file, delimiter='\t')
        if normalize:
            return [{key.lower().replace(' ', '_'): value for key, value in row.items()} for row in reader]
        return list(reader)

    def list_report_dates(self, report_name: ReportName, app_id: Optional[str] = None,
                          report_ids: Optional[List[str]] = None,
                          granularity: Granularity = Granularity.DAILY) -> List[str]:
        if not app_id and not report_ids:
            raise APIClientError("Either 'app_id' or 'report_ids' must be provided.")

        if not report_ids:
            response = self.list_reports(app_id=app_id, report_name=report_name, category=report_name.category)
            report_ids = Handler.extract_ids(response)

        dates: Set[str] = set()
        for report_id in report_ids:
            instances = self.read_list_of_instances_of_report(report_id, params={"filter[granularity]": granularity.value})
            available_dates = Handler.extract_attribute_values(instances, attribute='processingDate')
            if available_dates:
                dates.update(available_dates)
        return sorted(dates)

    def list_reports(self, app_id: str, category: Optional[Category] = None, report_name: Optional[ReportName] = None) -> List[Dict[str, Any]]:
        query_params = {}
        if category:
    ...
    def get_data(self, app_id: str, report_name: ReportName, granularity: Granularity, dates: Optional[Set[str]] = None) -> List[Dict[str, str]]:
        ...
        if not dates:
            dates = set(self.list_report_dates(report_name, report_ids=report_ids, granularity=granularity))
    ...
    def _fetch_report_ids(self, app_id: str, report_name: ReportName) -> List[str]:
        reports = self.list_reports(app_id, category=report_name.category, report_name=report_name)
        return Handler.extract_ids(reports)

    def _fetch_instance_ids(self, report_ids: List[str], granularity: Granularity, dates: Set[str]) -> List[str]:
        instance_ids: List[str] = []
        for report_id in report_ids:
            for date in dates:
                instances = self.read_list_of_instances_of_report(report_id, params={"filter[granularity]": granularity.value, "filter[processingDate]": date})
